...
public class ArithUtil {
     ...
     * @return 两个参数的积
     */
    public static double mul(BigDecimal b1, double s2) {
        BigDecimal b2 = new BigDecimal(Double.toString(s2));
        return b1.multiply(b2).doubleValue();
    }
     ...
     * @return 两个参数的积
     */
    public static BigDecimal mul(double s1, BigDecimal s2) {
        BigDecimal b1 = new BigDecimal(Double.toString(s1));
        return b1.multiply(s2);
    }
     ...
     * @param s1 被乘数
     * @param s2 乘数
     * @return 两个参数的积, 以普通字符串返回
     */
    public static String mulStr(double s1, double s2) {
        BigDecimal b1 = new BigDecimal(Double.toString(s1));
        BigDecimal b2 = new BigDecimal(Double.toString(s2));
    ...
    // BigDecimal b1=new
    // BigDecimal(Math.pow(d1.doubleValue(),1./d2.doubleValue()));
...
//		BigDecimal b1 = new BigDecimal(Math.pow(d1.doubleValue(),
//				d2.doubleValue()));
//		return b1.doubleValue();
//	}
    ...
    /**
     * 功能：提供（相对）精确的除法运算。当发生除不尽的情况时，由scale参数指 定精度，以后的数字四舍五入。
     *
     * @param s1    被除数
     * @param s2    除数
     ...
     * @param s2    除数
     * @param scale 表示表示需要精确到小数点以后几位。
     * @return 两个参数的商, 以普通字符串返回
     */
    public static String divStr(double s1, double s2, int scale) {
        if (scale < 0) {
            throw new IllegalArgumentException("精确度不能小于0！");
        }
    ...
    /**
     * 向下取值 功能：提供（相对）精确的除法运算。当发生除不尽的情况时，由scale参数指 定精度，以后的数字四舍五入。
     *
     * @param s1    被除数
     * @param s2    除数
     * @param scale 表示表示需要精确到小数点以后几位。
     ...
     * @return 两个参数的商, 以普通字符串返回
     */
    public static String divDownStr(double s1, double s2, int scale) {
        if (scale < 0) {
            throw new IllegalArgumentException("精确度不能小于0！");
        }
     ...
     * @param v     需要四舍五入的数字
     * @param scale 小数点后保留几位
     * @return 四舍五入后的结果, 以字符串形式，避免科学技术法显示
     */
     ...
     * @param planTotal
     * @param putIn
     * @param time      月份 年平均收益率 = (预计到期本金收益和 / 计划投资金额) ^ (1 / 投资年限) - 1
     * @return
     */
